...
export default function DoctorProfile() {
  ...
  const load = () => {
    axiosClient.get("/doctor/profile/me").then((r) => setProfile(r.data));
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axiosClient.put("/doctor/profile/me", { ...form, departmentId: Number(form.departmentId) });
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-6">Your doctor account details.</p>

      {!editing ? (
        <div className="bg-white rounded-xl shadow-sm border p-6 grid md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
          <Info label="Full Name" value={`Dr. ${profile.fullName}`} />
