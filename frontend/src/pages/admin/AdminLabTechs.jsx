      ...
      <div className="bg-white rounded-xl shadow-sm border p-6 mb-6">
        ...
        {newCreds && (
          <div className="bg-teal-50 border border-teal-300 rounded-md p-3 mb-3 text-sm flex justify-between items-center flex-wrap gap-2">
            ...
            <button onClick={() => downloadCreds(newCreds)} className="bg-teal-700 hover:bg-teal-800 text-white text-xs px-3 py-1.5 rounded-md whitespace-nowrap">
              Download as .txt
            </button>
          </div>
        )}
        ...
        <form onSubmit={addLabTech} className="flex gap-2 flex-wrap">
          ...
          <button className="bg-teal-700 hover:bg-teal-800 text-white px-4 py-2 rounded-md whitespace-nowrap">Create Account</button>
        </form>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
            <tr><th className="text-left px-4 py-3">Name</th><th className="text-left px-4 py-3">Email</th><th className="text-left px-4 py-3">Status</th></tr>
          </thead>
          <tbody>
            {labTechs.map((l) => (
              <tr key={l.id} className="border-t">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-1 rounded-full ${l.enabled ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                    {l.enabled ? "Active" : "Disabled"}
                  </span>
                </td>
              </tr>
            ))}
            {labTechs.length === 0 && <tr><td colSpan={3} className="px-4 py-6 text-center text-gray-400">No lab technicians yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
